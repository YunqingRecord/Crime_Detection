...
flag = 0   # 初始化 安全状态
vio_flag = 0  # 在状态是caution/NoWarning的前提条件下，只有当连续监测到2个vio_flag之后， 才会显示violence
vio_cnt = 0

peace_flag = 0  # 在状态是violence的前提条件下， 只有当连续检测到两个peace_flag，才会显示peace
peace_cnt = 0

ex_state = 0  # 显状态：只有达到更新要求的时候，才会进行更新的状态
im_state = 0  # 隐状态：每次预测都会进行更新的状态
im_state_pre = 0  # 上一隐状态，用来判别是不是连续判断同一结果，若是，则可以继续计数
X_vector_dim = 18
y_vector_dim = 3
input_shape = (20, 18)
# timesteps = 10
...
_activation = 'relu'
_optimizer = 'Adam'
model_weights_path = r"P:\CRIME\LearnDec\1\20190419_class2_frame20.h5"  # PATH  #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video', type=str, default='55.mp4')
            ...
            if len(humans) >= 2:  # 能检测到两个人的情况（包含很多子情况）
                for i in range(common.CocoPart.Background.value):
                    ...
                    if cnt % 360 is 0:
                        ...
                        test_sample_y_hat = model.predict(test_sample.reshape(1, 20, 18))
                        # print(test_sample_y_hat)

                        record = []

                        # head0 = [humans[0].body_parts[0], humans[1].body_parts[0]]
                        try:
                            head_dis = cal_dis(humans[0].body_parts[0], humans[1].body_parts[0])
                            print(head_dis)

                        except KeyError:
                            head_dis = 0
                        if test_sample_y_hat[0][0] >= 0.34 or head_dis > 0.25:
                            print("Primary detect No Warning")

                            im_state = 0
                            if im_state_pre != 2:  # 只要上一状态不是2，那么可以直接切换状态
                                flag = im_state    # 改变plot的状态
                                im_state_pre = im_state   # 无论怎样都要更新隐状态
                            else:
                                im_state_pre = im_state   # 否则，不更新状态, 只更新隐状态

                        if test_sample_y_hat[0][1] >= 0.34:
                            print("Primary detect Caution needed")
                        ...
                        if test_sample_y_hat[0][2] > 0.35:
                            print("Primary detect Violence Detected !!")

                            im_state = 2
                            if im_state == im_state_pre:  # 当前预测和上一预测相同，则更新flag
                                flag = im_state
                                im_state_pre = im_state   # 无论怎样都要更新隐状态
                            else:
                                im_state_pre = im_state   # 否则，不更新状态, 只更新隐状态
                ...
                if cnt % 360 is 0:
                    ...
                    if test_sample_y_hat[0][0] >= 0.34:
                        print("Primary detect No Warning")
                        im_state = 0
                        if im_state_pre != 2:  # 当前预测和上一预测相同，则更新flag
                            flag = im_state
                            im_state_pre = im_state  # 无论怎样都要更新隐状态
                        else:
                            im_state_pre = im_state  # 否则，不更新状态, 只更新隐状态
